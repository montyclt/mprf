...
use MPRF\Request\Filter\Filter;
...
class Response {
     ...
     * @param int $http_code
     */
    public function __construct($data, $http_code = self::HTTP_200_OK) {
     ...
     */
    public function dispatch($filter = null) {
        if ($filter) {
            $filter->filterData($this->data);
        }
